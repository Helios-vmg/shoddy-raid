...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Create {
            ...
            LibSRaid::create(&db_path, &disk_paths).context("Failed to create raid pool")?;

            println!(
                "Successfully initialized pool with {} disks.",
                disk_paths.len()
            );
        }
        Commands::AddFile {
        ...
        Commands::Scrub { db_path } => {
            let mut raid = LibSRaid::open(&db_path)
                .with_context(|| format!("Failed to open pool at {db_path:?}"))?;
            let result = raid.scrub(false).context("Failed to scrub pool")?;

            println!("Scrub completed:");
            println!(
                "  Repairable or repaired files: {}",
                result.repairable_or_repaired_files
            );
            println!(
                "  Repairable or repaired blocks: {}",
                result.repairable_or_repaired_blocks
            );
            println!("  Damaged files: {}", result.damaged_files.len());
            println!(
                "  Raw bytes read: {}",
                utils::format_bytes(result.raw_bytes_read)
            );
        ...
        Commands::CreateVdisk { vdisk_path, size } => {
            ...
            vdisk::VDisk::create(&vdisk_path, size_bytes).context("Failed to create vdisk")?;
        ...
        Commands::Info { db_path } => {
            let mut raid = LibSRaid::open(&db_path)
                .with_context(|| format!("Failed to open pool at {db_path:?}"))?;
            let info = raid.info().context("Failed to retrieve pool info")?;

            // Check if there is disk size mismatch
            let size_mismatch = info
                .disks
                .iter()
                .any(|d| d.size != info.smallest_disk_raw_bytes);
            ...
            if size_mismatch {
                ...
                println!(
                    "               by the smallest disk size: {}",
                    utils::format_bytes(info.smallest_disk_raw_bytes)
                );
            }
            println!("----------------------------------------------------");
            ...
            println!(
                "  Block size:          {} KiB ({})",
                info.raw_block_size / 1024,
                info.raw_block_size
            );
            println!(
                "  Total Superblocks:   {}",
                utils::add_thousands_separators(info.total_superblocks)
            );
            println!(
                "  Logical Capacity:    {}",
                utils::format_bytes(info.usable_pool_bytes)
            );
            println!(
                "  Physical Capacity:   {}",
                utils::format_bytes(info.raw_pool_bytes)
            );
